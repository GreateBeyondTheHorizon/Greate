...
public class TieredMillingRecipe extends TieredAbstractCrushingRecipe {
    ...
    public static TieredMillingRecipe convertNormalMilling(Recipe<?> recipe) {
        return new TieredProcessingRecipeBuilder<>(TieredMillingRecipe::new, recipe.getId()).withItemIngredients(recipe.getIngredients()).output(recipe.getResultItem(Minecraft.getInstance().level.registryAccess())).withItemOutputs(((ProcessingRecipe<?>) recipe).getRollableResults()).recipeTier(TIER.ULTRA_LOW).build();
    }
